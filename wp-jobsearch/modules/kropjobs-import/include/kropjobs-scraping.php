...
// main plugin class
class JobSearch_Kropjobs_Jobs_Scraping_Hooks {

    // hook things up
    public function __construct() {
        add_action('admin_menu', array($this, 'jobsearch_kropjobs_jobs_import_page'));
        
        add_action('wp_ajax_jobsearch_import_scraping_kropjobs_jobs', array($this, 'jobsearch_import_kropjobs_jobs'));
        
        add_action('jobsearch_jobimport_api_integrations_bklist_after', array($this, 'kropjobs_settings'), 15);
    }

    public function kropjobs_settings() {
        ...
        <div class="integrations-setins-section">
            <div class="integrations-setins-hding">
                <h2><?php esc_html_e('Kropjobs Jobs import Settings', 'wp-jobsearch') ?></h2>
            </div>
            <div class="jobsearch-element-field">
                <div class="elem-label">
                    <label><?php esc_html_e('Kropjobs Jobs import', 'wp-jobsearch') ?></label>
                </div>
                <div class="elem-field">
                    <div class="onoff-button"><input id="onoff-kropjobs-<?php echo($rand_id) ?>"
                                                        type="checkbox"<?php echo($kropjobs_import_jobs == 'on' ? ' checked' : '') ?>><label
                                for="onoff-kropjobs-<?php echo($rand_id) ?>"></label><input
                                type="hidden" name="jobsearch_integration_kropjobs_jobs"
                                value="<?php echo($kropjobs_import_jobs) ?>"></div>
                </div>
            </div>
        </div>
        <?php
    ...
    public function jobsearch_kropjobs_jobs_import_page() {
        $kropjobs_import_jobs = get_option('jobsearch_integration_kropjobs_jobs');
        if ($kropjobs_import_jobs == 'on') {
            add_submenu_page('edit.php?post_type=job', esc_html__('Import Kropjobs Jobs', 'wp-jobsearch'), esc_html__('Import Kropjobs Jobs', 'wp-jobsearch'), 'manage_options', 'import-kropjobs', array($this, 'import_jobs_form'));
        }
    }
    ...
    public function import_jobs_form() {
        ...
        ?>
        <hr>
        <div id="wrapper" class="jobsearch-post-settings jobsearch-kropjobs-import-sec">
            <h2><?php esc_html_e('Import Kropjobs Jobs', 'wp-jobsearch'); ?></h2>
            <form autocomplete="off" id="jobsearch-import-kropjobs" class="jobsearch-kropjobs" method="post" enctype="multipart/form-data">
                <?php
                wp_nonce_field('jobsearch-import-kropjobs-page', '_wpnonce-jobsearch-import-kropjobs-page');
                ?>
                <div class="jobsearch-element-field" style="display:none;">
                    ...
                    <div class="elem-field">
                        <?php
                        $field_params = array(
                            'force_std' => 'https://www.kropjobs.com/search',
                            'id' => 'page_url',
                            'cus_name' => 'page_url',
                ...
                <div class="jobsearch-element-field">
                    ...
                    <div class="elem-field">
                        <a href="javascript:void(0);" class="impkropjobs-submit-btn button" data-gtopage="1"><?php esc_html_e('Import Jobs', 'wp-jobsearch') ?></a>
                    </div>
            ...
            <script>
                jQuery('form#jobsearch-import-kropjobs .impkropjobs-submit-btn').on('click', function (e) {
                    e.preventDefault();
                    ...
                    formData.append('action', 'jobsearch_import_scraping_kropjobs_jobs');
    ...
    public function jobsearch_import_kropjobs_jobs() {
        global $jobsearch_plugin_options;
        $base_url = 'https://www.krop.com/creative-jobs';
        ...
        $det_base_url = 'https://www.krop.com';
        ...
        $query_arr = array();
        if ($keyword != '') {
            $query_arr[] = 'pos=' . urlencode($keyword);
        }

        if ($location != '') {
            $query_arr[] = 'loc=' . urlencode($location);
        }
        if ($page_num > 1) {
        ...
        //
        $base_url_transient = get_transient('jobsearch_kropjobs_import_base_url');
        $job_elems_transient = get_transient('jobsearch_kropjobs_import_job_elems');
        ...
        if ($base_url_transient == $base_url && $job_elems_transient != '') {
            ...
            $jobs_elements = $xpath->query("//a[contains(@class, 'posting')]");
        ...
        if ($jobs_elements_length > 0) {
            ...
            if ($base_url_transient != $base_url) {
                set_transient('jobsearch_kropjobs_import_base_url', $base_url, 900);
                $tosve_elements_arr = array();
                foreach ($jobs_elements as $job_element) {
                    $job_elem_href = $job_element->getAttribute('href');
                    $tosve_elements_arr[] = $job_elem_href;
                }
                set_transient('jobsearch_kropjobs_import_job_elems', $tosve_elements_arr, 900);
            }
            ...
            foreach ($jobs_elements as $job_element) {
                ...
                if ($found_elems_counter == $job_count) {
                    ...
                    if ($existing_id > 0) {
                        ...
                        $job_detail = @file_get_html($jk);

                        $job_title = '';

                        if ($job_detail) {
                            foreach ($job_detail->find('h1.job_title') as $job_title_html) {
                        ...
                        if ($job_title != '') {
                            ...
                            // }
                            foreach ($job_detail->find('div.job_benefits_list li.salary') as $job_salary_html) {
                                $job_salary = wp_kses($job_salary_html, array());
                            }

                            foreach ($job_detail->find('div.job_location') as $job_location) {
                                $job_location = wp_kses($job_location, array());
                                $job_location = trim($job_location);

                            }

                            foreach ($job_detail->find('div.job_description div') as $job_desc_html) {
                            ...
                            // $kropjobs_job_type = $job_detail->find('span.jobsearch-JobMetadataHeader-item');
                            // $kropjobs_job_type = isset($kropjobs_job_type[0]) ? $kropjobs_job_type[0] : '';

                            // if ($kropjobs_job_type != '') {
                            //     $kropjobs_job_type = wp_kses($kropjobs_job_type, array());
                            //     $kropjobs_job_type = str_replace(array('<', '>', '-', '!'), array('', '', '', ''), $kropjobs_job_type);
                            // }

                            $job_salary_min = '';
                            $job_salary_max = '';
                            $job_salary_type = 'type_1';
                            
                            if ($job_salary != '') {
                                $job_salary = str_replace(array(','), array(''), $job_salary);
                                $job_salary_parts = explode(' to ', $job_salary);
                                if (isset($job_salary_parts[0]) && isset($job_salary_parts[1])) {
                                    ...
                                    if(!empty($job_salary_parts[1])){
                                        $salary_part_1 = $job_salary_parts[1];
                                        $job_salary_parts_ = explode(' ', $salary_part_1);
                                        if(!empty($job_salary_parts_[1])){
                                            $job_salary_type = $job_salary_parts_[1];
                                        }
                                    }
                            ...
                            update_post_meta($post_id, 'jobsearch_job_referral', 'kropjobs', true);
                            ...
                            // if ($kropjobs_job_type != '') {
                            //     if (strpos($kropjobs_job_type, ',')) {
                            //         $kropjobs_job_types = explode(',', $kropjobs_job_type);
                                    
                            //         $type_term_ids = array();
                            //         foreach ($kropjobs_job_types as $the_job_type) {
                            ...
                            //     } else {
                            //         $type_term = get_term_by('name', $kropjobs_job_type, 'jobtype');
                            //         if (empty($type_term)) {
                            //             wp_insert_term($kropjobs_job_type, 'jobtype');
                            //             $type_term = get_term_by('name', $kropjobs_job_type, 'jobtype');
                            //         }
                    ...
                    if ($found_jobs > $job_count && $skiping_job === false) {
                        ...
                        ?>
                        <script>
                            var this_form = jQuery('form#jobsearch-import-kropjobs'),
                            ...
                            var request = jQuery.ajax({
                                ...
                                data: {
                                    ...
                                    kropjobs_num_jobs: '<?php echo (isset($kropjobs_page_nums) ? $kropjobs_page_nums : '') ?>',
                                    ...
                                    action: 'jobsearch_import_scraping_kropjobs_jobs'
                                },
                                dataType: "json"
                            });
...
return new JobSearch_Kropjobs_Jobs_Scraping_Hooks();
