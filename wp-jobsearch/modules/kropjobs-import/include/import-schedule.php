...
// main plugin class
class JobSearch_Kropjobs_Jobs_Scraping_Scheduler {
    ...
    public function schedule_jobs_form_opt() {
        ...
        if ($kropjobs_import_jobs == 'on') {
            ?>
            <option value="kropjobs"><?php esc_html_e('Kropjobs', 'wp-jobsearch') ?></option>
            <?php
        }
    }
    ...
    public function schedule_jobs_form_fields($import_from = '', $schedule_itm = '') {
        $kropjobs_import_jobs = get_option('jobsearch_integration_kropjobs_jobs');
        if ($kropjobs_import_jobs == 'on' && $import_from == 'kropjobs') {
                ...
                foreach ($jobs_elements as $job_element) {
                    ...
                    if ($existing_id > 0) {
                        ...
                        if ($job_title != '') {
                            ...
                            // }
                            foreach ($job_detail->find('div.description') as $job_desc_html) {
...
return new JobSearch_Kropjobs_Jobs_Scraping_Scheduler();
