...
class JobsearchGoogle
{
    ...
    private function set_access_tokes()
    {
        ...
        if ((isset($_GET['code']) && isset($_GET['scope'])) && !isset($_GET['state']) && !isset($_GET['redirect_from']) && !isset($_GET['jobsearch_instagram_login'])) {
            ...
            if (strpos($get_scope, 'googleapis') !== false) {
                ...
                wp_redirect(admin_url('admin-ajax.php?action=jobsearch_google_login'));
                exit();
            }
        }
    }
    ...
    public function renderShortcode()
    {
        ...
        if (isset($authUrl)) {
            echo '<li><a class="jobsearch-google-plus-bg" data-original-title="google" href="' . $authUrl . '"><div class="jobsearch-google-svg"><div></div></div>' . __('Sign in with Google', 'wp-jobsearch') . '</a></li>';
        }
    }
