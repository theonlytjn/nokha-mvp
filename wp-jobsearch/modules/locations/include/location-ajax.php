...
if (!class_exists('jobsearch_location_ajax')) {
    ...
    class jobsearch_location_ajax
    {
        ...
        public function getExistingLocationDetail($param_1, $param_2, $loc_type)
        {
            ...
            switch ($loc_type) {
                case "country":
                    $this->query = "SELECT * FROM `{$wpdb->prefix}jobsearch_countries` WHERE name = '" . ($param_1) . "' and code = '" . $param_2 . "'";
                    return $wpdb->get_results($this->query);
                    break;
                case "state":
                    $this->query = "SELECT * FROM `{$wpdb->prefix}jobsearch_states` WHERE state_name = '" . ($param_1) . "' and cntry_id = '" . $param_2 . "'";
                    return $wpdb->get_results($this->query);
                    break;
                case "city":
                    $this->query = "SELECT * FROM `{$wpdb->prefix}jobsearch_cities` WHERE city_name = '" . ($param_1) . "' and state_id = '" . $param_2 . "'";
                    return $wpdb->get_results($this->query);
                    break;
            }
        }
        ...
        public function jobsearch_locations_get_country_states_callback()
        {
            ...
            $states = $wpdb->get_results("SELECT * FROM `{$wpdb->prefix}jobsearch_states` WHERE cntry_id = '" . $country_id . "' ");
            ob_start(); ?>
        ...
        public function jobsearch_delete_cities_callback()
        {
            ...
            foreach ($totl_city_ids as $city_id) {
                //
                $this->query = "delete from `{$wpdb->prefix}jobsearch_cities` where city_id = '" . $city_id . "' ";
                $wpdb->query($this->query);
            }
        ...
        public function jobsearch_delete_states_callback()
        {
            ...
            foreach ($totl_state_ids as $state_id) {
                ...
                $this->query = "DELETE FROM `{$wpdb->prefix}jobsearch_states` WHERE state_id = '" . $state_id . "' ";
                $wpdb->query($this->query);
                //
                $this->query = "delete from `{$wpdb->prefix}jobsearch_cities` where state_id = '" . $state_id . "' ";
                $wpdb->query($this->query);
            }
        ...
        public function jobsearch_add_new_cities_callback()
        {
            ...
            foreach ($cities_list as $cities_info) {
                ...
                if ($cities_info['city_id'] == '') {
                    $this->query = "SELECT * FROM `{$wpdb->prefix}jobsearch_cities` where city_name = '" . $city_name . "' and cntry_id = '".$country_id."' and state_id = '".$state_id."' ";
                    ...
                    $this->query = "update `{$wpdb->prefix}jobsearch_cities` set city_name = '" . $city_name . "' where city_id = '" . $cities_info['city_id'] . "' ";
                    $wpdb->query($this->query);
                }
            }
        ...
        {
            global $wpdb;
            $this->query = "delete from `{$wpdb->prefix}jobsearch_countries` where cntry_id = '" . $_POST['cntry_id'] . "' ";
            $wpdb->query($this->query);
            //
            $this->query = "delete from `{$wpdb->prefix}jobsearch_states` where cntry_id = '" . $_POST['cntry_id'] . "' ";
            $wpdb->query($this->query);
            //
            $this->query = "delete from `{$wpdb->prefix}jobsearch_cities` where cntry_id = '" . $_POST['cntry_id'] . "' ";
            $wpdb->query($this->query);
        }
        ...
        {
            global $wpdb;
            $this->query = "delete from `{$wpdb->prefix}jobsearch_states` where state_id = '" . $_POST['state_id'] . "' ";
            $wpdb->query($this->query);
            //
            $this->query = "delete from `{$wpdb->prefix}jobsearch_cities` where state_id = '" . $_POST['state_id'] . "' ";
            $wpdb->query($this->query);
        }
        ...
        {
            global $wpdb;
            $this->query = "delete from `{$wpdb->prefix}jobsearch_cities` where city_id = '" . $_POST['city_id'] . "' ";
            $wpdb->query($this->query);
        }
        ...
        public function delete_dummy_state($country_id)
        {
            ...
            if (count($states) > 0) {
                ...
                if ($states[0]->state_name == $state_text) {
                    $this->query = "DELETE FROM `{$wpdb->prefix}jobsearch_states` WHERE state_name = '" . $state_text . "' and cntry_id = '" . $country_id . "' ";
        ...
        public function jobsearch_add_new_states_callback()
        {
            ...
            foreach ($states_list as $states_info) {
                ...
                if ($states_info['state_id'] == '') {
                    ...
                    $states = $wpdb->get_results("SELECT * FROM `{$wpdb->prefix}jobsearch_states` where state_name = '" . $state_name . "'");
                    ...
                    $this->query = "update `{$wpdb->prefix}jobsearch_states` set state_name = '" . $states_info['state_name'] . "' where state_id = '" . $states_info['state_id'] . "' ";
                    $wpdb->query($this->query);
                }
            }
            echo json_encode(array('status' => 'data_updated'));
        ...
        public function jobsearch_update_Country_Callback()
        {
            ...
            foreach ($countries_list as $contries_info) {
                $countries = $wpdb->get_results("SELECT * FROM `{$wpdb->prefix}jobsearch_countries` where name = '" . $contries_info['name'] . "' || code = '" . $contries_info['code'] . "' ");
                if (count($countries) === 0) {
                    ...
                    $this->query = "update `{$wpdb->prefix}jobsearch_countries`  set name = '" . $contries_info['name'] . "' where code = '" . $contries_info['code'] . "' ";
                    $wpdb->query($this->query);
                }
            }
            echo json_encode(array('status' => 'data_updated'));
        ...
        public function jobsearch_load_single_state_cities_data_callback()
        {
            ...
            $cities = $wpdb->get_results("SELECT * FROM `{$wpdb->prefix}jobsearch_cities` WHERE state_id = '" . $state_id . "' ORDER BY city_name");
        ...
        public function jobsearch_load_single_state_data_callback()
        {
            ...
            $states = $wpdb->get_results("SELECT * FROM `{$wpdb->prefix}jobsearch_states` WHERE cntry_id = '" . $country_id . "' ORDER BY state_name");
        ...
        public function jobsearch_load_single_country_data_callback()
        {
            global $wpdb;
            $country_id = isset($_POST['country_id']) ? $_POST['country_id'] : '';
            $countries = $wpdb->get_results("SELECT * FROM `{$wpdb->prefix}jobsearch_countries` where cntry_id = '" . $country_id . "' ");
        ...
        public function jobsearch_location_load_countries_data_callback()
        {
            ...
            if (isset($_POST['get_country_code']) && $_POST['get_country_code'] == true) {
                ...
                $countries = $wpdb->get_results("SELECT * FROM `{$wpdb->prefix}jobsearch_countries` where name = '" . $jobsearch_sloc_country . "'");
        ...
        public function jobsearch_location_load_cities_data_callback()
        {
            ...
            $this->query = "SELECT * FROM `{$wpdb->prefix}jobsearch_cities` WHERE state_id = '" . $state_id . "' ";
            if ($country_code != '') {
        ...
        {
            global $wpdb;
            $country_detail = $wpdb->get_results("SELECT * FROM `{$wpdb->prefix}jobsearch_countries` WHERE code = '" . $contry_code . "' ");
            if(!empty($country_detail[0]->cntry_id)){
